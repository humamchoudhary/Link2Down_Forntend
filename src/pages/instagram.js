...
export default function Instagram() {
  ...
  function getData() {
    ...
    axios
      .get(`${URL}/instagram/details?url=${url}`)
      .then((response) => {
  ...
  return (
    <>
      <Head>
        <title>All Downloader | Instagram</title>
        <meta name="description" content="Generated by create next app" />
        <meta name="viewport" content="width=device-width, initial-scale=1" />
      </Head>
      <Layout>
        <div className="border-insta border-2 rounded-md px-4 py-40 flex flex-col justify-center items-center gap-8 duration-300 transition-all">
          ...
          <div className="flex flex-row gap-2 border-2  2xl:w-4/5 border-insta rounded-md">
            <input
              className="bg-transparent w-4/5 2xl:w-11/12 focus:outline-none focus:text-insta active:outline-none px-4"
              placeholder="Paste your link here"
              value={url}
              onChange={(val) => setUrl(val.target.value)}
            />
            <div
              className="bg-insta  px-5 text-bg font-medium py-4 hover:cursor-pointer "
              onClick={() => getData()}
            >
              Download
            </div>
          ...
          {loadingData && (
            <div class="">
              <div class="load-3">
                <div className="bg-insta line"></div>
                <div className="bg-insta line"></div>
                <div className="bg-insta line"></div>
              </div>
            </div>
          ...
          {details["owner"] && (
            <div className="flex flex-col md:flex-row justify-center items-center gap-10">
              <div className=" flex justify-center items-center flex-col gap-5 md:w-1/3">
                <div className=" relative">
                  {details["__typename"] === "GraphVideo" && (
                    <div className=" absolute w-full h-full bg-slate-500 bg-opacity-50 flex justify-center items-center">
                      <AiOutlinePlayCircle className="text-bg text-3xl" />
                    </div>
                  )}
                  <ImageComponent
                    classname={"object-contain md:w-64 2xl:w-96"}
                    src={details["display_url"]}
                  />
                </div>
                {/* <ImageComponent src={details["display_url"]} /> */}

                <div className=" font-bold text-lg ">
                  {details["edge_media_to_caption"]["edges"][0]["node"][
                    "text"
                  ].slice(0, 60) + "..."}
          ...
          {details["owner"] && (
            <div className="flex flex-col justify-center items-center">
              {details["__typename"] === "GraphSidecar" && <div>Downloads</div>}
              {details["__typename"] === "GraphSidecar" ? (
                <div
                  className={`grid grid-cols-1 justify-center md:grid-cols-5 gap-12 md:gap-6`}
                >
                  {details["edge_sidecar_to_children"]["edges"].map((key) => {
                    // if (key["node"]["__typename"] === "GraphImage") {
                    return (
                      <div className=" relative flex flex-col gap-2 ">
                        <div className=" relative">
                          {key["node"]["__typename"] === "GraphVideo" && (
                            <div className=" absolute w-full h-full bg-slate-500 bg-opacity-50 flex justify-center items-center">
                              <AiOutlinePlayCircle className="text-bg text-3xl" />
                            </div>
                          )}
                          <ImageComponent
                            classname={"object-contain md:w-36 2xl:w-64"}
                            src={key["node"]["display_url"]}
                          />
                        </div>
                        <button
                          className="bg-yt text-bg px-2 rounded-md hover:cursor-pointer py-2 2xl:py-4"
                          onClick={() => {
                            if (key["node"]["__typename"] === "GraphVideo") {
                              download_video(key["node"]["video_url"]);
                            } else {
                              download_image(key["node"]["display_url"]);
                            }
                          }}
                        >
                          Download
                ...
                <button
                  className="bg-yt text-bg px-12 rounded-md hover:cursor-pointer py-2 2xl:py-4"
                  onClick={() => {
                    download_video(details["video_url"]);
                  }}
                >
                  Download
                </button>
                ...
                <button
                  className="bg-yt text-bg px-12 rounded-md hover:cursor-pointer py-2 2xl:py-4"
                  onClick={() => {
                    download_image(details["display_url"]);
                  }}
                >
                  Download
                </button>
        ...
        <div className="mt-8 max-w-3xl mx-auto">
          ...
          <div className="flex flex-wrap justify-center gap-10 mb-12">
            <div className="justify-items-center w-full sm:w-1/2 md:w-1/4 mb-4 px-1 py-4 border-2 border-insta rounded-lg">
              <p className="text-center ">Unlimited downloads</p>
              <p className="text-center ">12k, 8K, 4k Free Downloader</p>
            </div>
            <div className="w-full sm:w-1/2 md:w-1/4 mb-4 px-1 py-4 border-2 border-insta rounded-lg">
              <p className="text-center py-1">1000+ Supported Websites</p>
              <p className="text-center py-1">YouTube Private Videos</p>
            </div>
            <div className="w-full sm:w-1/2 md:w-1/4 mb-4 px-1 py-6 border-2 border-insta rounded-lg">
          ...
          <span className="font-semibold">Crisp and stunning:</span> MP4 in 8K,
          4K, 1440p, 1080p, 720p, 480p, 360p, 240p, and 144p
          <br />
          ...
          Range Videos
          <br />
          <span className="font-semibold">Silky smooth motion:</span> 30FPS and
          60FPS options
          <br />
          <br />
          ...
          <strong>
            <em>
              And that's not all! We've got you covered with additional popular
              video formats:
            </em>
          </strong>
          <br />
